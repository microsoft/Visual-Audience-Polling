...
@SuppressLint("NewApi")
public class CameraPoll extends Activity implements CvCameraViewListener2 {
	...
	*/
	...
	ArrayList<String> pList = TakeCameraAttendance.getPresentIds();
	ArrayList<String> aList = TakeCameraAttendance.getAbsentIds();
	ArrayList<String> eList = TakeCameraAttendance.getExtraIds();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.camera_poll);
	...
	protected Mat processIt(Mat tmp) {
		...
		// float buff[] = new float[(int) (tmp.total() * tmp.channels())];
		// tmp.get(0, 0, buff);
		// detectBlack method detects the contours and calculates the parameters
		Mat res1 = detectBlack(filtered, tmp);

		return res1;
	}

	private Mat detectBlack(Mat filtered, Mat greys) {
		...
		while (each1.hasNext()) {
			...
			if (Imgproc.contourArea(wrapper) < 6400
				...
				if ((sizeRect.height / sizeRect.width) > 0.7
					...
					if ((sizeRect.height * sizeRect.width) < 1.2 * Imgproc
							.contourArea(wrapper)) {
		...
		mat = chitraBanao(mat, greys, filtered);
		Long dr = System.nanoTime() - drs;
		Integer totCards = cardsTable.size();
		String logCd = frameCount.toString() + "      " + totCards.toString()
				+ "      " + cd.toString() + "         " + gr.toString()
				+ "         " + dr.toString();
		writeToFile("cd.txt", logCd);
		// writeToFile("gr.txt",gr);
		// writeToFile("dr.txt",dr);
		/*
		 * if(optionCount[0]!=null){ String A = optionCount[0].toString();
		 * Core.putText(mat, "A: "+A,new
		 * Point(40,210),Core.FONT_HERSHEY_SIMPLEX, 0.4, new Scalar(255,0,0),1);
		 * } if(optionCount[1]!=null){ String B = optionCount[1].toString();
		 * Core.putText(mat, "B: "+B,new
		 * Point(70,240),Core.FONT_HERSHEY_SIMPLEX, 0.4, new Scalar(255,0,0),1);
		 * } if(optionCount[2]!=null){ String C = optionCount[2].toString();
		 * Core.putText(mat, "C: "+C,new
		 * Point(100,210),Core.FONT_HERSHEY_SIMPLEX, 0.4, new
		 * Scalar(255,0,0),1); } if(optionCount[3]!=null){ String D =
		 * optionCount[3].toString(); Core.putText(mat, "D: "+D,new
		 * Point(130,240),Core.FONT_HERSHEY_SIMPLEX, 0.4, new
		 * Scalar(255,0,0),1); } // Core.putText(mat,
		 * "A: "+optionCount[0].toString
		 * ()+" | B: "+optionCount[1].toString()+" | C: "
		 * +optionCount[2].toString()+" | D: "+optionCount[3].toString(),new
		 * Point(40,120),Core.FONT_HERSHEY_SIMPLEX, 1, new Scalar(255,0,0),3);
		 */
		/*
		 * Point numCards = new Point(40,180); Integer detectedNum =
		 * answers.size(); Core.putText(mat,
		 * "Cards detected: "+detectedNum.toString
		 * (),numCards,Core.FONT_HERSHEY_SIMPLEX, 0.4, new Scalar(255,0,0),1);
		 */
		// findPehchaan(mat);
		return mat;
		// return filtered;
	...
	public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
		...
		/*
		 * Integer totalCards = tempAns.size(); if(frameCount == 100){ //Integer
		 * detectedNum = answers.size();
		 * 
		 * Set<Integer> keyAns = tempAns.keySet(); Iterator<Integer> itAns =
		 * keyAns.iterator(); while(itAns.hasNext()){ int each = itAns.next();
		 * ArrayList<Integer> ansList = tempAns.get(each); int max = 0; int
		 * maxIndex = 0; for(int j=0; j<4;j++){ if(ansList.get(j)>=max){ max =
		 * ansList.get(j); //maxIndex = j; } } maxIndex = ansList.indexOf(max);
		 * switch(maxIndex){ case 0: a++; break; case 1: b++; break; case 2:
		 * c++; break; case 3: d++; break;
		 * 
		 * }
		 * 
		 * } // frameCount = 0; detectedNum = answers.size(); //
		 * Core.putText(resMat, "Cards detected: "+totalCards.toString(),new
		 * Point(40,210),Core.FONT_HERSHEY_SIMPLEX, 0.4, new Scalar(255,0,0),1);
		 * 
		 * 
		 * }
		...
		// if(frameCount%50 == 0){
		/*
		 * Iterator<Point> itHole = holeList.iterator();
		 * while(itHole.hasNext()){ Point each = itHole.next();
		 * Core.putText(resMat, "o",each,Core.FONT_HERSHEY_SIMPLEX, 0.4, new
		 * Scalar(255,0,0),1); } Iterator<Integer> itId = idsList.iterator();
		 * Point test = new Point(10,10); while(itId.hasNext()){ Integer each =
		 * itId.next(); Core.putText(resMat,
		 * each.toString(),test,Core.FONT_HERSHEY_SIMPLEX, 0.4, new
		 * Scalar(255,0,0),1); }
		 */
		// }
		/*
		 ...
		 * }
		 * 
		 * } Integer net = a+b+c+d; Point numCards = new Point(40,180);
		 * Core.putText(resMat,
		 * "Cards detected: "+net.toString(),numCards,Core.FONT_HERSHEY_SIMPLEX,
		 * 0.4, new Scalar(255,0,0),1);
		 */
		frameCount++;
		areas.clear();
		per.clear();
	...
	private void writeToFile(String fileName, Object logData) {
		String content = logData.toString();
		// File file;
		try {
			FileWriter fileW;
			if (frameCount < 3) {
				fileW = new FileWriter("storage/sdcard1/Pictures/Second Sight/"
						+ fileName, false);
			} else {
				fileW = new FileWriter("storage/sdcard1/Pictures/Second Sight/"
						+ fileName, true);
				// fileW = new
				// FileWriter(Environment.getExternalStorageDirectory().toString()+"/"+fileName,true);
			}
			fileW.write(content + "\n");
			fileW.flush();
			fileW.close();
		...
		/*
		 * FileOutputStream outputStream; try { file = new
		 * File(Environment.getExternalStorageDirectory(), fileName);
		 * 
		 * outputStream = new FileOutputStream(file);
		 * outputStream.write(content.getBytes()); outputStream.close(); } catch
		 ...
		 * outputStream.write(string.getBytes()); outputStream.close(); } catch
		 * (Exception e) { e.printStackTrace(); }
		 */
		/*
		 * String data = logData.toString(); String filePath =
		 * "storage/sdcard1/Documents/CD.txt"; try { OutputStreamWriter
		 * outputStreamWriter = new OutputStreamWriter(openFileOutput(filePath,
